...
class ValidadorRegistro{
    ...
    private function validar_nombre($conexion,$nombre){
        ...
        if(strlen($nombre)>24){
            return "el nombre no puede ocupara mas de 24 caracteres.";
        }
    ...
    private function validar_email($conexion,$email){
        if(!$this->variable_iniciada($email)){
            return "debes proporcinal un email";
        }else{
            $this->email=$email;
        }
    ...
    private function validar_clave1($clave1){
     ...
     private function validar_clave2($clave2,$clave1){
         if(!$this->variable_iniciada($clave1)){
            return "debes rellenar la contrasena";
        }
        if(!$this->variable_iniciada($clave2)){
            return "debes repetir tu  contrasena";
        }
        if($clave1!==$clave2){
            return "ambars contrasenas deben conincidir.";
        }
        return "";
    }
